...
if(isset($_POST['save'])){
	...
	if (($stid=="")||($frdate=="")){
		...
		$sql_update = "update leaves set staff_id = '$stid', fromdate = '$frdate',todate = '$tdate',reason='$reason' where id = '$numberid'";
		$s = pg_query($connection, $sql_update);
		header("location:detail_user.php?id1=$stid");
	  }
}
echo '<form name="form1" method="post" action="edit_leaves_sub.php">';
echo '<h3>Leave Days';
echo '</h3>';
echo '<table>';
echo "<tr>";
