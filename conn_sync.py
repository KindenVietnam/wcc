...
def load_att_data():
	...
	cursor_ivs.execute("select id,sothe,thoigian from trangthaichamcong where datepart(month, thoigian) between %s and %s",(last_month, current_month))
	...
	conn_pg.commit()
	cursor_pg.execute("""INSERT INTO inout(inout_id, staff_id, checktime) (SELECT id,staff_id,checktime FROM temp_inout WHERE id >(SELECT max(inout_id) as lonnhat FROM inout))""")
